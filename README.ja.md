...
### 方法1：Windows ワンクリック起動（初心者推奨）

Docker によるシステム負荷や構成時のトラブルを避けるため、Windows ユーザー向けにワンクリックで起動できるスクリプトを提供しています。

**システム要件**: Windows 10 またはそれ以降。

#### 使用手順

1. 本リポジトリを空のフォルダにクローンします（ZIP でのダウンロードは更新を受け取れなくなるため非推奨）：
   `git clone https://github.com/1deaaa/spark-arc-studio`
2. **プロジェクトのルートディレクトリにある `start.bat` をダブルクリックします。**
3. 初回実行時は、ポータブル版 Python（約 40MB）と必要な依存関係が自動的にダウンロードされ、手動の操作なしでインストールされます。
4. インストール完了後、自動的にバックエンドサービスが起動します。
5. 2回目以降は、デプロイ完了のフラグを検出して**インストールをスキップし、直接サービスを起動**します。

アクセス用 URL: **<http://localhost:6688>** または Releases からダウンロードしたクライアント（推奨）。
...
GitHub Releases で提供されているクライアントは、あくまでも利便性のための「シェル（フロントエンド外殻）」であり、あなたのプライベートバックエンドに自動接続する機能はありません。**デスクトップ用またはモバイル用のクライアントをダウンロードした場合は、ログイン前にデフォルトのサーバーアドレスを実際のプライベートバックエンドのアドレスに変更してください**。デフォルトアドレスは開発者が管理する公式インスタンスを指しており、自己デプロイユーザーの直接利用には適していません。
...
* **スキルと MCP の境界**: AgentSkills は `search_skills` / `read_skill` / `read_skill_reference` 経由で執筆品質の参考資料としてオンデマンドで読み込まれ、システム前頭部を自動的に汚染することはありません。MCP インスピレーションポストは `/api/mcp` から `capture_inspiration` ツールを排他的に露出し、チャットエージェントのツールとは隔離されています。
...
このプロジェクトの最初のバージョンは、設計から開発、テストまでのすべての工程を私が一人で行いました。そのため、至らない点が多々あるかと思います。本職の都合上、個別の要望への即時対応やメンテナンスは遅れる場合がありますので、ぜひ皆様にプルリクエストや Issue などを通じて開発にご協力いただけますと幸いです。
