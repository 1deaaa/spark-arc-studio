...
SparkArc 并非简单的单轮对话工具，而是一个由多智能体协作的长文创作台。用户在日常使用中会产生高频、大上下文的模型调用，这意味着巨大的 API 消耗量：

*   **高频的 Agent 协同**：用户输入灵感后，后台的 Director 智能体将调度设定、大纲、执笔与逻辑审核等多个专家 Agent。单次创作流程即可触发数十次并发调用。
*   **深度长文本分析**：
    *   **Critic 审核**：对数万字的小说或剧本进行逻辑与人设审查，极其依赖长上下文的推理。
    *   **文风克隆分析**：每次分析都会对 30k tokens 以上的文本块进行多次回测与微调，Token 消耗量极大。
...
#### 4. 个人打赏
如果您是个人用户，单纯想支持创作者：
*   您可以通过 GitHub Sponsors 进行任意额度的单次或按月赞助，您的名字将展示在赞助者致谢墙中。
