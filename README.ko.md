...
| 단계 | 대응 역할 | 에이전트 | 기능 설명 |
| :--- | :--- | :--- | :--- |
| **0. 조율 및 스케줄링** | Director | **감독 에이전트** | 글로벌 엔트리 및 컨텍스트 제어자. LangGraph 멀티스텝 툴 호출을 통해 자율적으로 업무를 스케줄링하고 위임하며 진행 상황을 추적합니다. |
| **1. 기획 / 크리에이티브** | Logline / High Concept | **영감 에이전트** | 찰나의 스쳐 지나가는 영감을 포착하여 스토리 시드로 변환하고 확장합니다. |
| **2. 세계관/설정** | Story Bible / World Guide | **설정 전문가** | 물리 법칙, 마법/기술 계통, 세계의 역사·정치, 주요 등장인물 카드를 작성하여 스토리 전체의 논리적 일관성을 설계합니다. |
| **3. 템포/구조** | Beat Sheet / Treatment | **기획자 에이전트** | 스토리의 큰 뼈대를 정합니다. 막 구조를 단단히 고정하고 비트 시트와 장/화별 대강을 구성합니다. |
| **4. 집필** | Screenplay / Script | **작가 에이전트** | 실질적인 집필을 담당합니다. 인터랙티브 포맷과 순수 소설 포맷을 모두 지원합니다. |
| **5. 품질 보증 (QA)** | Script Doctor / Coverage | **비평가 & 스타일 클론** | 크리틱은 독자의 입장에서 AI 작성 패턴을 검출하고 등급을 부여합니다. 스타일 클론은 AI 특유의 고빈도 어휘를 제거합니다. |
| **6. 배포/연출** | Implementation / Assets | **브라우저 연출 / Unity SDK** | 극본을 고성능 런타임으로 컴파일하여 게임 내 대화 시스템, 연출 스케줄링, 퀘스트 트리거를 구동합니다. |
...
> ⚠️ **현 상태 안내**: 신호등 버스에 필요한 핵심 인프라가 모두 구현되어 UI에서 조작 가능합니다. 다만 에이전트 간 수평 자율 통신은 **예약 기능**으로 남겨져 있습니다——평가 결과, **주류 모델이 아직 다중 턴, 다중 롤, 장문맥 상호작용을 완전히 처리할 능력에 도달하지 못한 것**으로 확인되었습니다. 주류 모델이 충분한 복잡 추론 능력과 어텐션 능력에 도달하면 본 메커니즘이 공식 활성화되어, **수평 상호작용을 통한 창작 효율과 품질의 제2차 도약을 실현**합니다.
...
> 📗 텍스트 파싱 전략의 상세 내용은 [아키텍처 가이드라인 §9](docs/architecture.md#9-arc-格式解析策略)에 기재되어 있습니다.

### 소설 모드

인터랙티브 대화극 포맷 외에도, 일반적인 줄글 형태의 **순수 문학 소설** 집필 모드를 지원합니다. 프로젝트 설정 단에서 소설 모드로 전환할 시:

- 전문가 에이전트들이 소설에 더 적합한 문학적 스타일을 사용합니다
- 연출 단말이 깔끔하고 집중하기 좋은 소설 리더로 자동 전환됩니다
- 극본 에디터가 소설 뷰로 자동 전환됩니다

두 모드는 세계관, 캐릭터, 아웃라인, 비트 시트를 공유하며 최종 출력 포맷만 다릅니다.
