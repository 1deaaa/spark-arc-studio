...
@files_router.get('/api/story-memory/{project_name}/state')
async def get_story_memory_state(project_name: str, user: dict = Depends(get_current_user)):
    """返回项目 StoryMemory 的有界只读总览，供写作界面状态面板使用。"""
    try:
        user_id = str(user['user_id'])
        ...
        overview = StoryMemoryFacade(user_id, normalized).compose_state_overview()
        return {"success": True, "state": overview}
    except Exception as exc:
        return JSONResponse(status_code=500, content={"success": False, "message": f"获取故事记忆失败: {exc}"})
