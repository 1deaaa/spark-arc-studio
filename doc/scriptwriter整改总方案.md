...
补充状态：已完成一轮首版全接口回归测试（含新旧 ScriptWriter 接口与四域 agent 执行接口）。
...
状态：已完成（页面手动生成、MCP 捕获、Muse 工具调用已收敛到同一执行层）
...
状态：已完成（世界观页面生成与 Lorebook 工具改写已复用同一执行层）
...
状态：已完成（梗概 / 节拍 / 大纲页面生成与 Showrunner 工具改写已复用同一执行层）
...
当前状态：已完成。`/api/ai/single-node` 与 `/api/ai/multi-node` 已删除，前端主入口已切换到统一执行接口。
