...
from ._paths import get_runtime_file_path, get_package_file_path
...
def load_default_platform_configs_raw() -> Dict[str, Any]:
    ...
    config_path = get_runtime_file_path("llm_mgr_cfg.yaml", seed_from_package=True)
...
def save_default_platform_configs_raw(configs: Dict[str, Any]) -> str:
    """将平台配置原样写回 YAML 文件。"""
    config_path = get_runtime_file_path("llm_mgr_cfg.yaml", seed_from_package=False)
...
def _ensure_env_setup():
    ...
    if not key:
        gui_path = get_package_file_path("llm_mgr_cfg_gui.py")
