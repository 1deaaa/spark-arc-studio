...
def test_set_local_embedding_primes_startup_state_before_spawn(monkeypatch) -> None:
    ...
    monkeypatch.setattr(local_embedding, "start_local_embedding_service_background", fake_start_background)
...
def test_restore_local_embedding_uses_persisted_switch(monkeypatch) -> None:
    calls = {"start": 0}
    ...
    assert local_embedding.restore_local_embedding_service_from_settings() is True
    assert calls["start"] == 1


def test_restore_local_embedding_does_not_start_when_persisted_switch_is_off(monkeypatch) -> None:
    calls = {"start": 0}
    ...
    assert local_embedding.restore_local_embedding_service_from_settings() is False
    assert calls["start"] == 0


def test_cancelled_local_embedding_start_does_not_spawn_process(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        subprocess,
        "Popen",
        lambda *args, **kwargs: popen_calls.__setitem__("count", popen_calls["count"] + 1),
    )

    status = local_embedding.start_local_embedding_service(cancel_event=cancel_event)

    assert popen_calls["count"] == 0
