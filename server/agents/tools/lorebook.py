...
class RewriteAllCharactersInput(BaseModel):
    overwrite_content: str = Field(description='一批完整的角色设定文本。推荐 XML: <character><name>角色名</name><content>角色设定</content></character>；也支持 JSON: {"characters":[{"name":"角色名","content":"角色设定"}]}；或兼容旧的纯文本格式：角色名+空行+角色内容，多个角色用 --- 分隔。角色较多时应分批调用，通常每批最多约 5 个角色；默认会按名称更新已有角色并追加新角色，不会删除其他角色。5 个只是防止超长调用截断的软建议，必须根据每个角色的实际设定长度调整，绝不能为了凑批次或满足数量而压缩、删减角色设定，角色质量与完整度优先')
    append: bool = Field(default=True, description="角色集合写入策略。默认 true，按角色名更新已有角色并追加新角色，不删除本批未包含的角色；只有用户明确要求清空重做或整体替换全部普通角色时，才允许显式传 false")
...
def rewrite_all_characters(overwrite_content: str, append: bool = True) -> str:
