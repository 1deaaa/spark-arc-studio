...
const messages = {
  'zh-CN': {
    launcher: {
      ...
      localDeploy: {
        title: '启动本地后端',
        restartButton: '重新启动后端',
        restartTooltip: '启动本地后端服务',
        starting: '正在启动本地后端',
        help: '首次使用会将 main 源码下载到 APP 数据目录，准备便携 Python 与 Launcher 内置 Node.js，并自动安装依赖；整个过程可能需要几分钟。保持此窗口打开即可。',
        idle: '等待开始',
        running: '正在准备本地后端...',
  ...
  'en-US': {
    launcher: {
      ...
      localDeploy: {
        ...
        restartTooltip: 'Start the local backend service',
        starting: 'Starting Local Backend',
        help: 'On first use, SparkArc downloads main into the app data directory, prepares portable Python and the Launcher-provided Node.js runtime, then installs dependencies automatically. This can take a few minutes; keep this window open.',
        idle: 'Ready to start',
        ...
        retry: 'Retry',
        close: 'Close',
        checkUpdate: 'Check Local Service Updates',
    ...
    server: {
      ...
      errors: {
        ...
        openFailed: 'Unable to open the workspace: {detail}',
      },
    },
  },
  'ja-JP': {
    launcher: {
      ...
      localDeploy: {
        title: 'ローカルバックエンドを起動',
        restartButton: 'バックエンドを再起動',
        restartTooltip: 'ローカルバックエンドサービスを起動',
        starting: 'ローカルバックエンドを起動中',
        help: '初回は main のソースをアプリデータフォルダーへダウンロードし、ポータブル Python と Launcher 内蔵 Node.js を準備して、依存関係を自動でインストールします。数分かかる場合があるため、このウィンドウを開いたままにしてください。',
        idle: '開始待ち',
    ...
    server: {
      ...
      errors: {
        ...
        openFailed: 'ワークスペースを開けませんでした: {detail}',
      },
    },
  },
  'ko-KR': {
    launcher: {
      ...
      localDeploy: {
        title: '로컬 백엔드 시작',
        restartButton: '백엔드 다시 시작',
        restartTooltip: '로컬 백엔드 서비스 시작',
        starting: '로컬 백엔드 시작 중',
        help: '처음 사용할 때는 main 소스를 앱 데이터 폴더에 내려받고 휴대용 Python과 Launcher 내장 Node.js를 준비한 뒤 의존성을 자동 설치합니다. 몇 분 정도 걸릴 수 있으니 이 창을 열어 두세요.',
        idle: '시작 대기 중',
    ...
    server: {
      ...
      errors: {
        ...
        openFailed: '작업실을 열 수 없습니다: {detail}',
      },
    },
  },
} as const;
