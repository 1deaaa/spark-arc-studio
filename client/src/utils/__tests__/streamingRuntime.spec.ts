...
type BusEventCapture = { type: string; payload: GlobalLoadingPayload | undefined };

function createReaderFromString(text: string): ReadableStreamDefaultReader<Uint8Array> {
...
describe('createStreamingTask', () => {
  ...
  it('dispose emits hide event and unregisters cancel handler', () => {
    const payloads: BusEventCapture[] = [];
    const originalEmit = bus.emit.bind(bus);
    bus.emit = ((type: string, payload?: unknown) => {
      payloads.push({ type, payload: payload as GlobalLoadingPayload | undefined });
...
describe('consumeSSEReader', () => {
  it('parses SSE events and flushes trailing buffer', async () => {
    ...
    await consumeSSEReader(reader as ReadableStreamDefaultReader<Uint8Array>, {
      onEvent: (evt) => {
        events.push(evt);
      },
    });
