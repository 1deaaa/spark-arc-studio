...
### 방법 1: Windows 원클릭 시작 (초보자 추천)

Docker 구동에 따른 자원 부하와 초기 설정 시 발생할 수 있는 오류를 피하고자, Windows 사용자를 위한 원클릭 시작 스크립트를 제공합니다.

**시스템 권장 사양**: Windows 10 혹은 그 이상.

#### 실행 방법

1. 본 리포지토리를 빈 폴더에 클론합니다 (ZIP으로 직접 다운로드 시 업데이트 추적이 어렵습니다):
   `git clone https://github.com/1deaaa/spark-arc-studio`
2. **프로젝트 루트 디렉토리에 위치한 `start.bat` 파일을 더블 클릭합니다.**
3. 최초 실행 시 휴대용(Portable) Python 버전(약 40MB)과 필요한 모든 의존성 패키지를 사람의 수동 개입 없이 자동으로 감지하여 다운로드 및 설치합니다.
4. 설치가 성공적으로 완료되면 백엔드 서비스가 자동으로 실행됩니다.
5. 이후 재실행할 때는 배포 완료 플래그를 감지하여 **설치 단계를 건너뛰고 서비스만 즉시 구동**합니다.

접속 주소: **<http://localhost:6688>** 또는 Releases에서 다운로드한 클라이언트(권장).
...
GitHub Releases 페이지에서 배포되는 설치형 클라이언트는 편의성을 위해 프론트엔드 외형만 감싸놓은 껍데기(Shell)에 불과하며, 당신이 개인 구축한 독립 백엔드에 자동으로 접속하지 않습니다. **데스크톱용 혹은 모바일용 전용 앱을 받아 구동하기 전에 반드시 로그인 화면에서 기본 서버 주소를 사용자 소유의 실제 도메인/IP 주소로 교정해 주시기 바랍니다**. 기본 주소는 개발자가 공익 목적으로 유지하는 데모 주소를 향하므로 개별 데이터를 올리기 적합하지 않습니다.
