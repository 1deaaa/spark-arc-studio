...
SparkArc 规定：**每个专家 Agent 的提示词必须严格区分三种调用模态**，并通过同一个 `yaml` 的三个顶层字段承载。同一个 Agent 在不同触发路径下呈现的人格、输出格式与行为边界是完全不同的，这保证了“手动面板生成”“用户聊天调用”“导演自动委派”三条路径互不串味。
...
- **同一 Agent 三种人格**：用户聊天时 Muse 是“热情的头脑风暴搭子”；手动点按钮时 Muse 是“冷静的结构化解析对象”；被导演调度时 Muse 是“执行严格产出规范的自动化专家”。这三者不可互相污染。
- **规范与自由隔离**：`chat_system` 允许发散，`pipeline_system` 禁止发散。这避免了用户只想聊几句时反被格式捆死，也避免了导演委派时 Agent 自由发挥跑偏产出。
- **新增 Agent 的硬门槛**：任何新 Agent 必须同时定义三个字段、且 `pipeline_system` 必须**独立自闭合**（不能引用 `system` 字段的内容），否则在导演接管全自动流水线时会出现“Muse 被委派时开始构建世界观”这类模态串味 Bug。

贡献者请参阅 [AGENTS.md §4.5](AGENTS.md) 查看完整协议与新增 Agent 的自检清单。
...
## 写在最后
本项目从设计、开发、到测试，全程由本人(Mournight)一人完成，所以难免有许多瑕疵。欢迎各位积极提issue和PR。
这个项目最早是工作室内部用于游戏剧情系统开发使用，也是乘上了 AI 的风，独立成了单独的作品。
如协议定义，我无法禁止你拿去商用，但你要自担所有风险，尤其是内容合规、技术合规。本人只按原样提供合规的代码。
**实际运营者和其用户产生的任何生成式内容的合规问题均与本人无关**。**在此奉劝对公众开放服务的各位站长保持关闭匿名分享功能**。
以及，我希望大家能够共建这个项目，让它变得更好。遵守AGPLv3是底线。如果你对公众提供云服务，则必须开放修改后的完整源代码：
...
本项目代码基于 AGPL-3.0 开源，但 **"SparkArc" 名称、Logo、品牌视觉及相关标识不包含在代码授权范围内**。
