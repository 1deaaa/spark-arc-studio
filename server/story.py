...
def get_blueprint(project_name):
    ...
    try:
        user_id = request.current_user['user_id']
        project_path = get_project_path(user_id, project_name)
        blueprint_file = os.path.join(project_path, f"{project_name}.blueprint")
        ...
        return jsonify({"error": f"读取蓝图失败: {str(e)}"}), 500
...
def save_blueprint(project_name):
    ...
    try:
        ...
        return jsonify({"success": False, "message": f"保存蓝图失败: {str(e)}"}), 500