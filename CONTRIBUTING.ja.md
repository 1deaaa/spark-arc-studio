...
- **自己完結**：「通常生成と同じ / 形式は system と同一」等の参照表現は禁止。コード上、二つの system は**相互排他**で積み重ねられません。
- **出力規格を再記述**：`system` 中の中核構造制約（見出し、フィールド一覧、禁止事項、終端境界）を `pipeline_system` 内で復唱すること。例示は省略可、ハード制約は省略不可。
- **ツール落ちは必須明示**：完了時にどのツールをどの引数で呼ぶか、「ツール不呼出は失敗扱い」を明記。
- **受け手宣言**：冒頭で「あなたの受け手は監督であってユーザーではない」と宣言。
- **ブレスト系の修飾語厳禁**：「発散思考 / 慣例を打破 / 情熱的に」など、構造化出力と衝突する語気修飾を書かない。

`chat_system` は対話モードの人格と語調のみを規定し、出力フォーマットを強制しません。`system` は最も厳格な構造化規範を担います。以上いずれかに違反すると「監督委譲された Muse がインスピレーションではなく世界観を構築し始める」ようなモード混線が発生します。
