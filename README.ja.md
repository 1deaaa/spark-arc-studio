...
> 📢 **支援とフォローのお願い**：もしSparkArcが創作の参考や助けになりましたら、ぜひ画面右上にある **Star**（お気に入り登録・迷子防止）と **Watch**（「Custom -> Releases」を選択してアップデート通知を購読）をお願いいたします。個人開発の独立したオープンソースプロジェクトとして、皆様のStarとWatchはコミュニティにおける認知度の向上と、今後の継続的なアップデート・発展において極めて重要です。応援よろしくお願いいたします！
> 
> 🤝 **共同制作者**：デザインと宣伝における [<img src="https://github.com/wxwxwkai.png" width="20" style="border-radius:50%; vertical-align:middle;"/> @wxwxwkai](https://github.com/wxwxwkai) の多大な貢献に心より感謝いたします。これらの重要な取り組みがなければ、本プロジェクトが世に出ることはありませんでした。プロジェクトのイテレーションおよびコミュニティ管理は [<img src="https://github.com/wxwxwkai.png" width="20" style="border-radius:50%; vertical-align:middle;"/> @wxwxwkai](https://github.com/wxwxwkai) が主導し、[<img src="https://github.com/1deaaa.png" width="20" style="border-radius:50%; vertical-align:middle;"/> @1deaaa](https://github.com/1deaaa) も継続的に技術サポートを提供いたします。
...
| 段階 | 対応ロール | Agent | 役割 |
| :-- | :-- | :-- | :-- |
| 0. 調停 | Director | **監督** | LangGraph ベースの多段ツール呼出自律調整、タスク委譲、自動執筆起動、進捗追跡 |
| 1. 企画 | Logline / High Concept | **着想** | 着想の種を拡張し方向性を作る |
| 2. 設定 | Story Bible / World Guide | **設定専門家** | 世界観ルール、設定、人物基盤を整備 |
| 3. 構造 | Beat Sheet / Treatment | **構成プランナー** | ビートと章・シーン骨格を生成 |
| 4. 執筆 | Screenplay / Script | **執筆脚本家** | シーン本文を生成; インタラクティブ脚本と小説の二形態出力に対応 |
| 5. 品質 | Script Doctor / Coverage | **論理審査 & 文風克隆** | AI 臭・論理破綻・整合性ズレを検出。GraphRAG はオプションで追加可能 |
| 6. 出力 | Implementation / Assets | **ブラウザ演出 / Unity SDK** | 脚本を高性能ランタイムにコンパイルし、ゲーム内対話・演出スケジューリング・クエストトリガーを駆動 |
...
> ⚠️ **現在の状態**：完全なインフラが実装され UI からアクセス可能ですが、Agent 間の水平自律通信は**予約機能**です——評価の結果、**主流モデルはまだ多ターン・多ロール・長文脈のインタラクションを完全に処理する能力に達していません**。主流モデルが十分な複雑推論能力とアテンション能力に到達した際、本メカニズムは正式に有効化され、**水平インタラクションによる創作効率と品質の第二の飛躍を実現します**。
...
> 📘 完全な解析戦略：[アーキテクチャ詳細 §9](docs/architecture.md#9-arc-格式解析策略)

### 小説モード

インタラクティブ脚本フォーマットに加え、**純文学小説**出力モードをサポート。プロジェクトが小説モードに切り替わると：

- 専門家エージェントは小説により適した文学的スタイルを使用
- 演出端末はクリーンで集中しやすい小説リーダーに自動切替
- 脚本エディタは小説ビューに自動切替
...
もしあなたが API プロキシプロバイダー、アグリゲーター、または GPU スプライヤーであれば、私たちの [**スポンサーとビジネス協力ガイド**](.github/SUPPORT.md)（※現在は中国語・英語のみ）をご覧ください。開発とテストに必要な API クレジットのご支援と引き換えに、セルフホストユーザー向けの「デフォルト設定配信」など、露出効果の高いプロモーションチャネルを提供しています。**プロジェクトの高速な開発とアップデートを維持するために、皆様のご支援を心よりお待ちしております。**
